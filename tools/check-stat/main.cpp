...
static cl::OptionCategory checkStatCategory{"check-stat options"};

static cl::opt<std::string> databasePath{
    "p", cl::desc{"Path to compilation database"}, cl::Optional,
    cl::cat{checkStatCategory}};

static cl::opt<std::string> directCompiler{
    cl::Positional, cl::desc{"[-- <compiler>"}, cl::cat{checkStatCategory},
    cl::init("")};
...
static void processFile(clang::tooling::CompilationDatabase const &database,
    ...
    auto frontendFactory =
        clang::tooling::newFrontendActionFactory<CheckStatAction>();
...
int main(int argc, char const **argv) {
    ...
    cl::HideUnrelatedOptions(checkStatCategory);
