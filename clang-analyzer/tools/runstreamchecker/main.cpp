...
static cl::opt<std::string> directCompiler{
    cl::Positional, cl::desc{"[-- <compiler>"}, cl::cat{streamcheckerCategory},
    cl::init("")};
...
class StaticAnalyzerAction : public clang::ento::AnalysisAction {
   protected:
    std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
        ...
        analyzerOpts->CheckersAndPackages.push_back(
            {CHECKER_PLUGIN_NAME, true});
   ...
   private:
    std::string getPluginPath() const {
        ...
        constexpr const char *path =
            CMAKE_INSTALL_PREFIX "/lib/" PLUGIN_LIBRARY;
        ...
        if (!path || !llvm::sys::fs::exists(path)) {
            llvm::report_fatal_error(
                "Plugin runner compiled without a working plugin path.");
        }
        return path;
    }
};
...
static void processFile(clang::tooling::CompilationDatabase const &database,
    ...
    auto includeInserter = clang::tooling::getInsertArgumentAdjuster(
        include, clang::tooling::ArgumentInsertPosition::BEGIN);
